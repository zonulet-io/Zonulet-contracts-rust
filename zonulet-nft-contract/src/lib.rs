...
impl Contract {
    ...
    pub fn new_default_meta(owner_id: ValidAccountId, treasury_id: ValidAccountId) -> Self {
        Self::new(
            ...
            NFTContractMetadata {
                ...
                name: "Zonu by Zonulet".to_string(),
                symbol: "ZONU".to_string(),
                icon: Some(DATA_IMAGE_SVG_ZONU_ICON.to_string()),
    ...
    pub fn nft_create_series(
        ...
        assert_eq!(env::predecessor_account_id(), self.tokens.owner_id, "Zonulet: Only owner");

        let token_series_id = format!("{}", (self.token_series_by_id.len() + 1));

        assert!(
            self.token_series_by_id.get(&token_series_id).is_none(),
            "Zonulet: duplicate token_series_id"
        );

        let title = token_metadata.title.clone();
        assert!(title.is_some(), "Zonulet: token_metadata.title is required");
        ...
        assert!(total_accounts <= 10, "Zonulet: royalty exceeds 10 accounts");

        assert!(
            total_perpetual <= 9000,
            "Zonulet Exceeds maximum royalty -> 9000",
        );
    ...
    pub fn nft_buy(
        ...
        let token_series = self.token_series_by_id.get(&token_series_id).expect("Zonulet: Token series not exist");
        let price: u128 = token_series.price.expect("Zonulet: not for sale");
        let attached_deposit = env::attached_deposit();
        assert!(
            attached_deposit >= price,
            "Zonulet: attached deposit is less than price : {}",
            price
        );
    ...
    pub fn nft_mint(
        ...
        let token_series = self.token_series_by_id.get(&token_series_id).expect("Zonulet: Token series not exist");
        assert_eq!(env::predecessor_account_id(), token_series.creator_id, "Zonulet: not creator");
        let token_id: TokenId = self._nft_mint_series(token_series_id, receiver_id.to_string());
    ...
    fn _nft_mint_series(
        ...
        let mut token_series = self.token_series_by_id.get(&token_series_id).expect("Zonulet: Token series not exist");
        assert!(
            token_series.is_mintable,
            "Zonulet: Token series is not mintable"
        );
    ...
    pub fn nft_set_series_non_mintable(&mut self, token_series_id: TokenSeriesId) {
        ...
        assert_eq!(
            env::predecessor_account_id(),
            token_series.creator_id,
            "Zonulet: Creator only"
        );

        assert_eq!(
            token_series.is_mintable,
            true,
            "Zonulet: already non-mintable"
        );

        assert_eq!(
            token_series.metadata.copies,
            None,
            "Zonulet: decrease supply if copies not null"
        );
    ...
    pub fn nft_decrease_series_copies(
        ...
        assert!(
            ...
            "Zonulet: cannot decrease supply, already minted : {}", minted_copies
        );
    ...
    pub fn buy_mint_bundle(
        ...
        let mut mint_bundle = self.mint_bundles.get(&mint_bundle_id).expect(
            "Zonulet: Mint bundle does not exist or already finished"
        );

        assert_eq!(env::predecessor_account_id(), receiver_id.to_string(), "Zonulet: Can only buy for caller");

        let price = mint_bundle.price.expect("Zonulet: Mint bundle hasn't started yet");
        assert!(
            env::attached_deposit() > price,
            "Zonulet: Attached deposit lower than mint price"
        );
        ...
        if let Some(limit_buy) = mint_bundle.limit_buy {
            ...
            assert!(
                ...
                "Zonulet: Mint exhausted for account_id {}",
                receiver_id
            );
...
mod tests {
    ...
    fn test_new() {
        ...
        assert_eq!(contract.nft_metadata().icon.unwrap(), DATA_IMAGE_SVG_ZONU_ICON.to_string());
    }
    ...
    #[test]
    #[should_panic(expected = "Zonulet: Token series is not mintable")]
    ...
    #[test]
    #[should_panic(expected = "Zonulet: cannot decrease supply, already minted : 2")]
    ...
    #[test]
    #[should_panic(expected = "Zonulet: not for sale")]
    ...
    #[test]
    #[should_panic(expected = "Zonulet: Mint bundle does not exist or already finished")]
    ...
    #[test]
    #[should_panic(expected = "Zonulet: Mint exhausted for account_id charlie")]
